document.addEventListener("DOMContentLoaded", function() {
  ...
  function calcularPerdas() {
    ...
    const detalhesArray = []; // Array para guardar detalhes do cálculo
    ...
    detalhesArray.push(`<li>Perda do Cabo: ${perdaCabo.toFixed(2)} dB (${cabos} metros x ${perdaPorMetro.toFixed(3)} dB/m)</li>`);
  
    // Ajustar os valores dependendo da faixa de frequência:
    // TX: 5 a 85 MHz; RX: 105 a 999 MHz
    if (frequencia >= 5 && frequencia <= 85) {
      tx -= perdaCabo;
      detalhesArray.push(`<li>Aplicado na TX (frequência ${frequencia} MHz)</li>`);
    }
    if (frequencia >= 105 && frequencia <= 999) {
      rx -= perdaCabo;
      detalhesArray.push(`<li>Aplicado na RX (frequência ${frequencia} MHz)</li>`);
    }
  
    // Aplicar ajustes dos componentes selecionados
